...
describe("frontend load performance wiring", () => {
  it("lazy-loads dashboard, kitchen, food trucks, setup, and billing routes", async () => {
    ...
    assert.match(source, /lazy\(\(\) => import\("@\/pages\/dashboard\/business\/kitchen"\)\)/);
    assert.match(source, /lazy\(\(\) => import\("@\/pages\/dashboard\/business\/billing"\)\)/);
    assert.match(source, /lazy\(\(\) => import\("@\/pages\/dashboard\/admin\/overview"\)\)/);
    assert.match(source, /lazy\(\(\) => import\("@\/pages\/setup"\)\)/);
    assert.match(source, /<Suspense fallback=\{<RoutePageLoader/);
    assert.doesNotMatch(source, /import FoodTrucks from "@\/pages\/food-trucks"/);
    assert.doesNotMatch(source, /import BusinessKitchen from "@\/pages\/dashboard\/business\/kitchen"/);
  });
  ...
  it("loads Leaflet only from the map canvas chunk", async () => {
    ...
    assert.match(mapSection, /lazy\(\(\) => import\("\.\/food-truck-map-canvas"\)\)/);
  ...
  it("debounces business directory search input", async () => {
    const source = await readFile(new URL("../pages/businesses.tsx", import.meta.url), "utf8");
