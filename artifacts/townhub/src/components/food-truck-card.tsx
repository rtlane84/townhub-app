...
export function FoodTruckTodayCard({ truck }: { truck: FoodTruckLocationWithBusiness }) {
  ...
  return (
    <Card className={cn("group flex h-full w-full flex-col overflow-hidden rounded-[1.75rem]", CARD_INTERACTIVE)}>
      <Link href={storefrontHref} className="block min-w-0">
        ...
        />
        <CardContent className="flex flex-1 flex-col px-5 pb-3 pt-9">
          ...
          {description ? (
            <p className="mb-2 line-clamp-2 flex-1 text-sm leading-relaxed text-muted-foreground">{description}</p>
          ) : null}
        </CardContent>
      </Link>
      <CardContent className="mt-auto flex flex-wrap gap-2 px-5 pb-5 pt-0">
