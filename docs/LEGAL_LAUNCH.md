...
This is TownHaven's operational launch checklist, not legal or tax advice. The platform cannot accept live orders until the owner records the approvals and evidence below.
...
- [ ] A West Virginia sales-tax CPA has issued a written determination covering TownHaven's marketplace-facilitator/merchant-of-record treatment, taxable transactions, destination sourcing, registrations, returns, and monthly Stripe-to-ledger reconciliation.
...
- [ ] Every customer receipt/confirmation clearly identifies the business, order reference, items, tax, delivery fee, total, payment status, and support path. The business handles fulfillment and product/service disputes; TownHaven handles platform, sign-in, and payment-status issues.
- [ ] Subscription billing is the only TownHaven revenue model for the Clay pilot. Do not activate a customer-order platform fee or change tax/payment language until counsel and the CPA approve the updated model.
