...
| Terms of Use (EULA) URL | `https://townhaven.io/terms-of-service` (optional Apple standard EULA vs custom) |
...
Staging mirrors: `https://staging.townhaven.io/privacy-policy`, `/terms-of-service`.
...
| User ID | Yes | App Functionality | Clerk / TownHaven account id |
...
| Payment Info | **No** (recommended) | — | Card numbers enter Stripe Checkout / Connect hosted UI; TownHaven stores payment status + provider ids, not full card data. If Apple’s UI forces a payment row, declare only what leaves the device and say not used for tracking. |
...
- TownHaven is **not directed to children under 13** (stated in privacy policy).
...
**Published 2026-07-15** on app **TownHaven Local** (`6791258844`) by Ronald Lane.
...
1. [ ] Read [privacy-policy](https://townhaven.io/privacy-policy) and [terms](https://townhaven.io/terms-of-service); edit copy if counsel requires changes.
