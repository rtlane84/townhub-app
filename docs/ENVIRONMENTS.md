...
- The production project has the reviewed schema, storage bucket, and locked-down public access applied. The Supabase production dashboard currently reports the Free plan and explicitly says project backups are not included; production backups and a restore drill remain release blockers. Production pilot data must still be introduced deliberately.
