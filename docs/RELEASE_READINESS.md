...
| OPS-002 | P1 | Monitoring | Sentry/health/Pino exist, but external uptime, log drain, alert routing, and release verification are unconfirmed | End-to-end staging and production monitoring with tested alerts and runbook | In progress: Better Stack uptime live; owner ack’d test alert; Errors DSNs cut over on Railway + Cloudflare (redeploys verified); HTTP log source ready; remaining = Railway Locomotive log drain (+ optional free status page) |
