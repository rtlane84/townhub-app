...
| OPS-001 | P0 | Recovery | Production backup history and restore drill are not verified | Managed backups enabled and a restore drill recorded before real orders | Open: the Supabase production project dashboard currently reports the Free plan, which does not include project backups; no backup history or restore drill can be recorded until the project is upgraded or an alternate managed backup provider is configured |
| OPS-002 | P1 | Monitoring | Sentry/health/Pino exist, but external uptime, log drain, alert routing, and release verification are unconfirmed | End-to-end staging and production monitoring with tested alerts and runbook | In progress: `https://api.townhub.io/health` and `https://api-staging.townhub.io/health` returned 200 on 2026-07-14; external uptime checks, log drain, alert destinations, and alert tests remain |
