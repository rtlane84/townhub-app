...
| ENV-001 | P0 | Environments | Staging and production isolation is not implemented or verified | Separate domains, data, credentials, webhooks, storage, identity, payments, push, and monitoring | In progress: domain/API/Clerk/Stripe isolation evidenced (prod 0 businesses vs staging 3; frontends bake correct API hosts); `wrangler.toml` staging/production envs added; Cloudflare Builds still main-only on `townhub-app` and `townhub-production` has zero Builds — dual Builds projects + Railway branch watches remain |
...
| OPS-002 | P1 | Monitoring | Sentry/health/Pino exist, but external uptime, log drain, alert routing, and release verification are unconfirmed | End-to-end staging and production monitoring with tested alerts and runbook | In progress: GitHub Actions `.github/workflows/uptime-health-check.yml` added (5-minute probes of staging/production API+frontend); human Better Stack/UptimeRobot alerts, log drain, and ack tests remain |
