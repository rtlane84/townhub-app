...
| ENV-001 | P0 | Environments | Staging and production isolation is not implemented or verified | Separate domains, data, credentials, webhooks, storage, identity, payments, push, and monitoring | In progress: separate domains, Supabase projects, Railway API environments, Cloudflare Workers (`townhub-app` / `townhub-production`), Clerk instances, and Stripe live/test credentials are provisioned; 2026-07-15 health 200s on staging/production frontends+APIs; Railway/Cloudflare branch watch matrix (`develop`→staging, `main`→production) still needs dashboard confirmation |
...
| OPS-002 | P1 | Monitoring | Sentry/health/Pino exist, but external uptime, log drain, alert routing, and release verification are unconfirmed | End-to-end staging and production monitoring with tested alerts and runbook | In progress: API `/health` 200 on 2026-07-14 and 2026-07-15 for staging+production; frontends `townhub.io` / `staging.townhub.io` also 200 on 2026-07-15; external uptime checks, log drain, alert destinations, and alert tests remain |
