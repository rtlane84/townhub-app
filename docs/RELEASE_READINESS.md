...
| OPS-001 | P0 | Recovery | Production backup history and restore drill are not verified | Managed backups enabled and a restore drill recorded before real orders | In progress: repository-owned Supabase-to-R2 workflow added; R2 bucket, GitHub secrets, successful scheduled run, and restore drill remain |
