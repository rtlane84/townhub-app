...
| OPS-002 | P1 | Monitoring | Better Stack receives errors and the production API health monitor alerts the owner, but the 2026-07-17 audit found sensitive exception text, no current Telemetry source, one-monitor plan limits, and no backup team member | Deploy and verify Sentry sanitization; recreate/rotate the Railway log source; add production frontend coverage or document an approved alternative; add and test backup routing | In progress: local privacy/auth remediation is implemented and tested; external log-source recovery, expanded monitor coverage, backup recipient, deployment, and live verification remain |
