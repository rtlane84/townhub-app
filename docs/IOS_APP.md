...
| Kitchen ticket print | Desktop uses `window.print()`. On native iOS, Business Hub → order detail shares an HTML ticket via `@capacitor/share` + `@capacitor/filesystem` so Print/AirPrint can open from the system share sheet (WKWebView `window.print()` is not required). Canceling the share sheet is not treated as an app error. |
