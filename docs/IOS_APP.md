...
| Email/password asks for a verification code on iOS | Expected on new devices when Clerk Client Trust is on — enter the email OTP after password. Custom form must call `prepareSecondFactor` / `attemptSecondFactor`. |
