...
| Kitchen ticket print | Desktop / mobile browser: Print Ticket opens the system print dialog for the ticket only (hidden iframe). Native iOS: Cap `KitchenPrinter` plugin presents AirPrint via `UIPrintInteractionController` (not the share sheet). Canceling the print dialog is not treated as an app error. Requires an iOS rebuild after native plugin changes. |
